...
List<UserVO> allUsers = (List<UserVO>) request.getAttribute("allUsers");
...
<meta name="keywords" content="admin, mypage, html">
		...
		<div class="mypage-body">
			<div class="tabs">
				...
				<button class="tablinks"
					onclick="openTab(event, 'comment-management')">회원관리</button>
				<button class="tablinks active" id="defaultOpen"
					onclick="openTab(event, 'user-info')">관리자정보</button>
			</div>
						...
						<tbody>
							<c:forEach var="u" items="${allUsers}">
								<tr>
									...
									<td>
										<button onclick="editUser(${u.user_seq})">수정</button>
										<button onclick="deleteUser(${u.user_seq})">삭제</button>
									</td>
								</tr>
							</c:forEach>
						</tbody>
			...
			<div id="user-info" class="tabcontent">
				...
				<div class="info">
					<label for="nickname">Nickname:</label> <input type="text"
						id="nickname" name="nickname" value="<%=user.getUsername()%>"
						disabled>
					<button type="button" id="checkNickname" class="check-duplicate">중복
						검사</button>
					<label for="id">ID:</label> <input type="text" id="id" name="id"
						value="<%=user.getUserid()%>" disabled>
					<button type="button" id="checkId" class="check-duplicate">중복
						검사</button>
					<label for="email">Email:</label>
					<div class="email-input">
						<input type="text" id="userid" name="userid"
							placeholder="Username" value="<%=emailUsername%>" disabled>
						<span>@</span> <select id="emailDomain" name="emailDomain"
							disabled>
							...
							<option value="hanmail.net"
								<%="hanmail.net".equals(emailDomain) ? "selected" : ""%>>hanmail.net</option>
							<option value="other">Other</option>
						</select> <input type="text" id="otherDomain" name="otherDomain"
							placeholder="Other Domain" style="display: none;" disabled>
					</div>
					<button type="button" id="checkEmail" class="check-duplicate">중복
						검사</button>
					<label for="password">Password:</label> <input type="password"
						id="password" name="password" value="<%=user.getPassword()%>"
						disabled> <br> <label for="signup-date">Signup
						Date:</label> <input type="text" id="signup-date" name="signup-date"
						value="<%=user.getCreated_date()%>" disabled readonly> <br>
					<label for="last-update">Last Update:</label> <input type="text"
						id="last-update" name="last-update"
						value="<%=user.getUpdated_date()%>" disabled readonly> <br>
					<label for="profile-image">Profile Image:</label>
					<div class="profile-image">
						<img src="img/authors/1.jpg" alt="Profile Image">
						<div class="file-upload-wrapper" id="file-upload-wrapper"
							style="display: none;">
							<label for="file-upload" class="file-upload-label">Choose
								File</label> <input type="file" id="file-upload" name="profileImage">
				...
				<div class="buttons">
					...
					<button id="save-btn" class="btn" style="display: none"
						onclick="saveEdit()">확인</button>
	...
	<script>
        ...
        function deleteUser(userSeq) {
            if (confirm('정말로 이 사용자를 삭제하시겠습니까?')) {
                $.post('UserServlet', { action: 'delete', seq: userSeq }, function(response) {
                    if (response.success) {
                        alert('사용자가 삭제되었습니다.');
                        location.reload();
                    } else {
